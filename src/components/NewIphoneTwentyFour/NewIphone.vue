<template>
	<div class="new-iphone-main" v-if="get_info">
		...
		<div class="title">
			<p class="title-large">积分抵现专区</p>
			<p class="title-small">潮流数码各有不同</p>
		</div>
		<div class="old-iphone">
			<img v-for="item in index_ad_list.integral_goods" :key="item.id" :src="item.ad_img"
...
<script>
    export default {
                        ...
                        this.get_info.ad_list.forEach(item => {
                            switch (item.position_type) {
                                ...
                                case 'Mac':
                                    ...
                                    break;
                                case '积分商品':
                                    result.integral_goods.push(item);
                                    break;
                                case '尾图':
                                    result.floor = item;
                                    break;
        ...
        methods: {
            getIndexAd() {
                this.$fetch("get_index_info", {into_type: 'new_iphone_twenty_four'}).then((msg) => {
                    if (msg) {
                        this.get_info = msg;
                    }
